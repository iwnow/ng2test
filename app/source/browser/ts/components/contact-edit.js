...
var C2cContactEdit = (function () {
    ...
    C2cContactEdit.prototype.ngOnInit = function () {
        ...
        //photoloader
        var infile = document.getElementById('photoloader');
        infile.value = '';
    };
    ...
    C2cContactEdit.prototype.changeFile = function ($event) {
        var _this = this;
        try {
            var data = $event && $event.target && $event.target.files && $event.target.files[0];
            if (!data)
                return;
            ...
            fr.onload = function (ev) {
                _this.model.avatar = ev.target.result;
            };
            fr.readAsDataURL(file);
        }
        catch (e) {
            this._locator.getService('IEventService').emit({
                data: e,
                key: all_3.Descriptors.Exceptions
            });
        }
    };
    __decorate([
